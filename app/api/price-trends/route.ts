...
export async function GET() {
  try {
    ...
    // Updated Mapping to include all requested comparisons
    const materialMapping: Record<string, string> = {
      ...
      // Sulphur & Sulphuric Acid Comparison
    ...
    // Format expected: MonthYear,Material,Price
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      // Handle potential comma within quotes if necessary, but based on your file 
      // simple split by comma seems sufficient as no material names have commas.
      const [dateStr, rawMaterial, priceStr] = row.split(",")

      // Check if this is a material we want to display
      const mappedName = materialMapping[rawMaterial]
      if (mappedName) {
        const price = parseFloat(priceStr)
        
        // Format date from YYYY-MM-DD to YYYY-MM for grouping consistency
        // e.g., "2010-01-01" -> "2010-01"
        const formattedDate = dateStr.length >= 7 ? dateStr.substring(0, 7) : dateStr
    ...
    // Get unique materials for legend/filter
